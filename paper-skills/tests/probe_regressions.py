...
one. Run with:
  ...
  python3 tests/probe_regressions.py --skill-root . --run-dir /tmp/paper_probe
...
def run(skill_root, script, *args):
    p = subprocess.run(
        [sys.executable, os.path.join(skill_root, "paper-review", "scripts", script)] + list(args),
...
def main():
    ...
    src = read(os.path.join(root, "paper-review", "scripts", "convert_corpus.py"))
