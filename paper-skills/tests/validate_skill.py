...
Usage:
  python3 tests/validate_skill.py --skill-root . --run-dir /tmp/paper_validate
...
def run(skill_root, script, *args):
    py = os.path.join(skill_root, "paper-review", "scripts", script)
...
def main():
    ...
    p = subprocess.run([sys.executable, os.path.join(root, "paper-review", "scripts",
    ...
    occ_script = os.path.join(root, "paper-review", "scripts", "extract_occurrences.py")
    ...
    rev_skill = read(os.path.join(root, "paper-revise", "SKILL.md"))
    rev_rules = read(os.path.join(root, "paper-revise", "references", "edit_rules.md"))
    ledger = read(os.path.join(root, "paper-revise", "references", "ledger.md"))
    sweeps = read(os.path.join(root, "paper-review", "references", "sweeps.md"))
    rev_sk = read(os.path.join(root, "paper-review", "SKILL.md"))
    check("D1 README has no dead skill paths",
          "~/.codex/skills/paper-review" not in readme and "~/.codex/skills/paper-revise" not in readme)
    check("D2 README drops the missing eval-HTML promise",
          "eval-review-iteration-1.html" not in readme)
    check("D3 paper-revise acceptance does not say A1–A8",
    ...
    check("D15 all extractors know the 'Reference list' heading",
          all("reference\\s+list" in read(os.path.join(root, "paper-review", "scripts", s))
    ...
    _rt_path = os.path.join(root, "paper-revise", "scripts", "revision_token.py")
    ...
    led = read(os.path.join(root, "paper-revise", "references", "ledger.md"))
    er = read(os.path.join(root, "paper-revise", "references", "edit_rules.md"))
