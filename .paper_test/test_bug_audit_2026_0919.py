...
Run:  python3 .paper_test/test_bug_audit_2026_0919.py

Every check names the candidate id from the audit reports under
`paper_audit_data/2026-0919-1336-potential-bug-report/`. Only *confirmed* defects
...
spec = importlib.util.spec_from_file_location("paper_audit", str(WS / "paper_pipeline.py"))
nb = importlib.util.module_from_spec(spec)
sys.modules["paper_audit"] = nb
...
def test_c01_python310_parse():
    ...
    proc = subprocess.run(
        [py, "-c", "import ast,sys;ast.parse(open(sys.argv[1],encoding='utf-8').read())",
         str(WS / "paper_pipeline.py")], capture_output=True, text=True)
    check("C01 paper_pipeline.py parses on Python 3.10", proc.returncode == 0,
...
def test_c03_length_anchoring():
    ...
    for name, want in (("plain.md", 200), ("prose.md", want_prose), ("star.md", 200)):
        proc = run_py(WS / "paper-skills/paper-review/scripts/count_words.py", tmp / name,
...
def test_c04_cover_letter():
    tmp = scratch("paper_audit_cover_")
    write(tmp / "letter.md",
    ...
    check("C04 the date-first cover letter gets an M19 row",
          ...
          str(rows[:1]))
    proc = run_py(WS / "paper-skills/paper-review/scripts/count_words.py", tmp / "letter.md",
...
def test_c05_c06_revision_token():
    legacy = scratch("paper_audit_legacy_")
    ...
    refs = scratch("paper_audit_refs_")
    ...
    spec2 = importlib.util.spec_from_file_location(
        "rt", str(WS / "paper-skills/paper-revise/scripts/revision_token.py"))
...
def test_c07_empty_ledger():
    tmp = scratch("paper_audit_ledger_")
    root = tmp / "root"
...
def test_c08_counter_renames():
    normal = scratch("paper_audit_counter_")
    ...
    clash = scratch("paper_audit_clash_")
...
def test_c09_token_tool_missing_dir():
    proc = run_py(WS / "paper-skills/paper-revise/scripts/revision_token.py",
                  "/tmp/paper-audit-does-not-exist-xyz")
...
def test_c12_citations_uncited():
    work = scratch("paper_audit_cit_")
    write(work / "corpus" / "manuscript.txt",
          "We measured [1] and [2].\n\nReferences\n\n1. A\n2. B\n3. C\n")
    write(work / "corpus" / "cover_letter.txt", "See [3] for context.\n")
    out = work / "out"
    proc = run_py(WS / "paper-skills/paper-review/scripts/extract_citations.py",
                  "--work", work, "--out", out)
    artifact = ""
...
def test_c13_rtf_surrogates():
    cc_spec = importlib.util.spec_from_file_location(
        "cc", str(WS / "paper-skills/paper-review/scripts/convert_corpus.py"))
    cc = importlib.util.module_from_spec(cc_spec)
    cc_spec.loader.exec_module(cc)
    tmp = scratch("paper_audit_rtf_")
...
def test_c14_roman_lookalikes():
    ...
    out = work / "out"
    run_py(WS / "paper-skills/paper-review/scripts/extract_acronyms.py", "--work", work, "--out", out)
    artifact = ""
    for p in sorted(out.rglob("*")):
...
def test_c15_guess_role():
    cc_spec = importlib.util.spec_from_file_location(
        "ccr", str(WS / "paper-skills/paper-review/scripts/convert_corpus.py"))
    cc = importlib.util.module_from_spec(cc_spec)
    cc_spec.loader.exec_module(cc)
...
def test_c17_redlines_adapter():
    fake = scratch("paper_audit_redlines_")
    ...
    proc = run_py(WS / "paper_redlines_adapter.py", base, revised, out, env=env)
    ...
    fake2 = scratch("paper_audit_redlines2_")
    ...
    proc2 = run_py(WS / "paper_redlines_adapter.py", base, revised, out2, env=env2)
...
def test_c18_caption_subtraction():
    ...
    proc = run_py(WS / "paper-skills/paper-review/scripts/count_words.py", tmp / "ms.md",
                  "--section", "main-text", "--json")
...
spec = importlib.util.spec_from_file_location("paper_race", sys.argv[1])
nb = importlib.util.module_from_spec(spec)
sys.modules["paper_race"] = nb
spec.loader.exec_module(nb)
...
def test_m01_lock_reclaim_race():
    ...
    procs = [subprocess.Popen([sys.executable, str(child), str(WS / "paper_pipeline.py"),
...
REV_SCRIPTS = WS / "paper-skills" / "paper-review" / "scripts"
...
def test_l02_l04_l05_l09_l11():
    ...
    tmp = scratch("paper_audit_l5_")
    src = tmp / "source"
    src.mkdir()
    write(src / "ms.md", "Abstract\n\nhello world\n\nMethods\n\nx\n")
    root = tmp / "root"
    subprocess.run([sys.executable, str(WS / "paper_pipeline.py"), "setup", "--source", str(src),
    ...
    work2 = _corpus(scratch("paper_audit_l7c_"),
...
def test_l25_cover_letter_and_occurrence_guards():
    ...
    tmp = scratch("paper_audit_l25_")
    out = tmp / "out.docx"
    out.symlink_to(tmp / "missing-target.docx")
    adapter = _load_script("redlines_l25", WS / "paper_redlines_adapter.py")
    base = tmp / "base.docx"
    ...
    tmp = scratch("paper_audit_letter_")
