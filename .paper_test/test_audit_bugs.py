...
Run:  python3 .paper_test/test_audit_bugs.py
...
spec = importlib.util.spec_from_file_location("paper_bugs", str(WS / "paper_pipeline.py"))
nb = importlib.util.module_from_spec(spec)
sys.modules["paper_bugs"] = nb
...
def test_pipeline_lock_excludes_concurrent_writers():
    ...
    holder_src = (
        ...
        "import importlib.util\n"
        f"spec = importlib.util.spec_from_file_location('np', {str(WS / 'paper_pipeline.py')!r})\n"
    ...
    try:
        ...
        proc = subprocess.run([sys.executable, str(WS / "paper_pipeline.py"), "retry",
...
def test_decide_uses_the_recorded_plan():
    ...
    tmp2 = scratch("paper_bug_d2_")
    source = tmp2 / "source"
    write(source / "manuscript-b.md", "title\n")
    write(source / "refs-b.bib", "x\n")
    root = tmp2 / "root"
    setup = [sys.executable, str(WS / "paper_pipeline.py"), "setup", "--source", str(source),
