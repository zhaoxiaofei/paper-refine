...
Run:  python3 .paper_test/test_docx_converter.py
...
Asserts:
  ...
  * optionally (`PAPER_TEST_DOCX_MCP=1`), a real stdio handshake with the server
...
spec = importlib.util.spec_from_file_location("paper_mcp", str(WS / "paper_pipeline.py"))
nb = importlib.util.module_from_spec(spec)
sys.modules["paper_mcp"] = nb
    ...
    try:
        send({"jsonrpc": "2.0", "id": 1, "method": "initialize",
              "params": {"protocolVersion": "2024-11-05", "capabilities": {},
                         "clientInfo": {"name": "paper-test", "version": "0"}}})
        init = read_one(20)
...
def test_codex_exec_mcp_approval():
    ...
    tmp = scratch("paper_mcp_approval_")
    script = tmp / "server" / "index.js"
    script.parent.mkdir(parents=True)
    script.write_text("// fake mcp server\n", encoding="utf-8")
    cfg = tmp / "config.toml"
...
def test_live_server_if_available():
    ...
    if os.environ.get("PAPER_TEST_DOCX_MCP") != "1":
        skip("live docx->pdf conversion",
             "set PAPER_TEST_DOCX_MCP=1 to run it (needs Microsoft Word / WSL interop)")
        return
    tmp = scratch("paper_mcp_conv_")
