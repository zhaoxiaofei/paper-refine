...
Run:  python3 .paper_test/test_design_audit_2026_0919.py
...
spec = importlib.util.spec_from_file_location("paper_design_audit", str(WS / "paper_pipeline.py"))
nb = importlib.util.module_from_spec(spec)
sys.modules["paper_design_audit"] = nb
spec.loader.exec_module(nb)

COUNT_WORDS = WS / "paper-skills/paper-review/scripts/count_words.py"
FAILS = []
TMPDIRS = []
...
def test_m18_coverage_contract():
    ...
    check("D6 the header docstring matches the postcheck",
          ...
          (WS / "paper_pipeline.py").read_text(encoding="utf-8"))
...
def test_fallback_prompt_contract():
    ...
    prompt = (WS / "paper-skills/prompts/identify_issues.prompt.md").read_text(encoding="utf-8")
    skill = (WS / "paper-skills/paper-review/SKILL.md").read_text(encoding="utf-8")
    sweeps = (WS / "paper-skills/paper-review/references/sweeps.md").read_text(encoding="utf-8")
