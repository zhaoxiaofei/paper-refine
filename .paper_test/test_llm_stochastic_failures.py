...
Run:  python3 .paper_test/test_llm_stochastic_failures.py

`PAPER_WS` retargets the harness at a baseline copy; most cases fail there (the
...
spec = importlib.util.spec_from_file_location("paper_stoch", str(WS / "paper_pipeline.py"))
nb = importlib.util.module_from_spec(spec)
sys.modules["paper_stoch"] = nb
...
def new_case(prefix="paper_stoch_"):
...
def main() -> int:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="paper_stoch_"))
    ctx = build_root(tmp)
    ...
    def judge_case(mutate):
        tmp = Path(tempfile.mkdtemp(prefix="paper_stoch_j_"))
        ctx = build_root(tmp)
    ...
    def review_case(mutate):
        tmp = Path(tempfile.mkdtemp(prefix="paper_stoch_r_"))
        ctx = build_root(tmp)
        rid = "r1_a2_review"
        sb = ctx.runs_dir / rid
        sb.mkdir(parents=True)
