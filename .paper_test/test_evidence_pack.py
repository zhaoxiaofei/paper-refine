...
Run:  python3 .paper_test/test_evidence_pack.py
...
spec = importlib.util.spec_from_file_location("paper_evidence", str(WS / "paper_pipeline.py"))
nb = importlib.util.module_from_spec(spec)
sys.modules["paper_evidence"] = nb
spec.loader.exec_module(nb)
fmt_spec = importlib.util.spec_from_file_location("paper_fmt_ev", str(WS / "paper_docx_format.py"))
fmt = importlib.util.module_from_spec(fmt_spec)
sys.modules["paper_fmt_ev"] = fmt
...
def test_prompts():
    ...
    check("the judge prompt still tells the judge to derive its own rows",
          "paper_docx_format.py scan" in prompts["judge"]
