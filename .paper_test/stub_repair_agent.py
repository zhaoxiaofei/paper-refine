...
this wrapper decides from the prompt what it is:
  ...
  * a REPAIR prompt (`ARTIFACT REPAIR — SCOPED SESSION`): repair ONE bookkeeping
    ...
    Variants, all driven by the environment:
      PAPER_REPAIR_STUB_NOFIX=1  change nothing (the repair does not clear the
                               problem: the attempt must fail like any other)
      PAPER_REPAIR_STUB_EVIL=1   ALSO edit a file outside that stage's scope
                               (`review/findings.json`, or a manuscript document
                               inside the package): the guard must reject it
      PAPER_REPAIR_STUB_DROP=1   delete the last seeded row of a DECISION table
                               instead of filling it (the guard must reject a
                               table "fixed" by removing what it had to dispose)
      PAPER_REPAIR_STUB_DROP_OTHER=1
  ...
  * any other prompt: delegate to `stub_agent.py`, i.e. a complete stage
    deliverable, and then -- when PAPER_REPAIR_STUB_BAD matches this run id --
...
MODE = os.environ.get("PAPER_REPAIR_STUB_BAD_MODE", "content")
...
def break_stage(sb: Path, stage: str) -> str:
    ...
    if stage == "review":
        if os.environ.get("PAPER_REPAIR_STUB_M1B"):
...
def main() -> int:
    ...
    if "ARTIFACT REPAIR" in prompt[:2000]:
        ...
        if os.environ.get("PAPER_REPAIR_STUB_DROP"):
            ...
            return 0
        if os.environ.get("PAPER_REPAIR_STUB_DROP_OTHER"):
            ...
            if other.is_file():
                ...
                other.write_text("\n".join(lines) + "\n", encoding="utf-8")
            return 0
        note = "left the bookkeeping alone (PAPER_REPAIR_STUB_NOFIX)"
        if not os.environ.get("PAPER_REPAIR_STUB_NOFIX"):
            note = do_repair(sb, name)
        if os.environ.get("PAPER_REPAIR_STUB_EVIL"):
            note += "; " + go_out_of_scope(sb, name)
        print(f"stub repair ({stage or 'unknown stage'}): {note}")
        return 0
    rc = stub.main()
    bad = os.environ.get("PAPER_REPAIR_STUB_BAD") or ""
