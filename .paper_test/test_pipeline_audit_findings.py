...
`paper_audit_data/PIPELINE_AUDIT_FINDINGS.md`. When a finding is fixed, the
matching check must be inverted (see the note in each section) -- it is a
regression guard for the defect, not for the fix.

Run:  python3 .paper_test/test_pipeline_audit_findings.py
"""
...
def main() -> int:
    ...
    #   paper_audit_data/repros/test_audit_findings_prepatch.py (F1a-F1d).
    # =================================================================
    print("== F1. judged views carry no version-order metadata ==")
    tmp = Path(tempfile.mkdtemp(prefix="paper_f1_"))
    try:
        ctx = build_root(tmp)
        ...
        tmp1 = Path(tempfile.mkdtemp(prefix="paper_f1e_"))
    ...
    #   paper_audit_data/repros/test_audit_findings_prepatch.py (F2a/F2b/F2b').
    # =================================================================
    ...
    for salt in ("root-secret-A", "root-secret-B"):
        tmp = Path(tempfile.mkdtemp(prefix="paper_salt_"))
        ctx = build_root(tmp)
    ...
    #   paper_audit_data/repros/test_audit_findings_prepatch.py (the claim passed
    #   with nothing rendered).
    # =================================================================
    print("== F3. visual-inspection claims are evidenced ==")
    tmp = Path(tempfile.mkdtemp(prefix="paper_f3_"))
    try:
    ...
    #   paper_audit_data/repros/test_audit_findings_prepatch.py (the regenerated
    #   prompt was byte-identical to the first attempt's).
    # =================================================================
    print("== F4. retry prompts carry the previous failure ==")
    tmp = Path(tempfile.mkdtemp(prefix="paper_f4_"))
    try:
        ctx = build_root(tmp)
    ...
    #   paper_audit_data/repros/test_audit_findings_prepatch.py.
    # =================================================================
    print("== F5. judge/pinned corpora exclude the agent's bookkeeping ==")
    tmp = Path(tempfile.mkdtemp(prefix="paper_f5_"))
    try:
        ctx = build_root(tmp)
