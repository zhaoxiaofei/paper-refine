...
def test_journal_emphasis():
    ...
    tmp = scratch("paper_fmt_journal_")
...
def test_scan():
    ...
    empty = scratch("paper_fmt_empty_")
...
def test_cli():
    ...
    check("`scan` exits 0 and writes JSON", scan.returncode == 0
          ...
          scan.stdout[-200:])
    strict = subprocess.run([sys.executable, str(WS / "paper_docx_format.py"), "scan",
                             str(tmp), "--strict"], capture_output=True, text=True)
    check("`scan --strict` exits 1 when findings exist", strict.returncode == 1)
    fix = subprocess.run([sys.executable, str(WS / "paper_docx_format.py"), "fix", str(docx),
    ...
    same = subprocess.run([sys.executable, str(WS / "paper_docx_format.py"), "fix", str(docx),
...
def test_pipeline_wiring():
    ...
    for stage, text in prompts.items():
        check(f"the {stage} prompt carries the M20 formatting mandate",
              "M20" in text and "paper_docx_format.py" in text, text[text.find("M20") - 40:][:120])
    judge = nb.judge_prompt(Path(tmp) / "sb", "r1_judge_t1_j1", 1, "t1", 1, 3, ["v1"])
    check("the judge prompt carries the M20 formatting clause",
          "M20" in judge and "paper_docx_format.py" in judge)


def _pipeline_module():
    import importlib.util as ilu
    spec = ilu.spec_from_file_location("paper_pl_fmt", str(WS / "paper_pipeline.py"))
    nb = ilu.module_from_spec(spec)
    sys.modules["paper_pl_fmt"] = nb
    spec.loader.exec_module(nb)
    return nb
...
def test_setup_normalization():
    ...
    check("the companion module is copied into the root",
          (root / "paper_docx_format.py").is_file())
    # opt-out keeps the copy byte-identical to the source
    root2 = tmp / "root-off"
    proc2 = subprocess.run([sys.executable, str(WS / "paper_pipeline.py"), "setup",
