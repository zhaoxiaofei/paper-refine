...
Run:  python3 .paper_test/test_attempt_history.py
`PAPER_WS` retargets the suite at a baseline (pre-fix -> red) tree.
"""
...
spec = importlib.util.spec_from_file_location("paper_hist", str(WS / "paper_pipeline.py"))
nb = importlib.util.module_from_spec(spec)
sys.modules["paper_hist"] = nb
...
ctx = build_root(scratch("paper_hist_b_"))
...
ctx2 = build_root(scratch("paper_hist_f_"))
ctx2.cfg["rounds"] = 1
...
ctx3 = build_root(scratch("paper_hist_g_"))
...
root_h = scratch("paper_hist_h_")
log_path = nb.start_run_log("run", root_h, ["paper_pipeline.py", "run", "--root", str(root_h)])
check("H1 start_run_log opens reports/<cmd>-<stamp>.log with a header",
      log_path.is_file() and log_path.parent.name == "reports"
      and "paper_pipeline run" in log_path.read_text(encoding="utf-8")
...
ctx_h = build_root(scratch("paper_hist_h2_"))
...
inner = nb.start_run_log("retry", ctx_h.root, ["paper_pipeline.py", "retry"])
...
root_j2 = scratch("paper_hist_j_")
src_j = root_j2 / "source"
write(src_j / "manuscript-b.md", "title\n")
subprocess.run([sys.executable, str(WS / "paper_pipeline.py"), "setup", "--source", str(src_j),
