...
Run:  python3 .paper_test/test_acronym_longform.py
`PAPER_WS` retargets the harness at a baseline copy (pre-fix -> red),
`PAPER_SKILLS` at another skills pack.
"""
...
def sweep(files, name="case"):
    ...
    tmp = scratch(f"paper_acr_{name}_")
...
# --------------------------------------------------------------------------
sw = (SKILLS / "paper-review" / "references" / "sweeps.md").read_text(encoding="utf-8")
er = (SKILLS / "paper-revise" / "references" / "edit_rules.md").read_text(encoding="utf-8")
...
check("E4 the M1 script-table line documents the M1b audit",
      "M1b long-form audit" in (SKILLS / "paper-review" / "SKILL.md").read_text(encoding="utf-8")
