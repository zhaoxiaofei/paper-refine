...
content-derived version ID. `nbt-revise`'s R2 now says: give each revision
...
`nbt_pipeline.py`'s revision, rewrite and integration directives carry the same
...
Aligns the skills with the current `nbt_pipeline.py` policy surface.
...
- **Length rule (M19, always on).** Replaces the blanket "abstract/main-text
  ...
  never a journal requirement. New bundled `nbt-review/scripts/count_words.py`
...
- **Zotero policy (rule E2).** Replaces the absolute "never touch a citation
  ...
  create/delete/bulk edits. nbt-review still never writes to the library.
...
`.nbt_test/test_acronym_longform.py`; `tests/validate_skill.py` now has 42
...
- **Two matcher bugs found while testing the fix**: a hyphenated long form that
  ...
  genuine residue.
- **Review spec (`nbt-review/references/sweeps.md`)**: new finding rule **(k)**
...
- **Revise spec (`nbt-revise/references/edit_rules.md`)**: new rule **P1a** —
...
- **Orchestrator (`nbt_pipeline.py`)**: `REVISE_DIRECTIVES` item 8 carries
...
- `nbt-review/SKILL.md`: OUT/WORK may not live inside SUBMISSION_DIR; the
...
- `nbt-revise/SKILL.md` + `references/ledger.md`: acceptance check now says
...
- A second, byte-identical copy of this package is installed at
  `~/.codex/skills/nbt-skills/`. Codex registers skills by name, so the two
