...
name, so duplicate `nbt-review/` or `nbt-revise/` directories silently compete
...
```
nbt-review  (identify_issues)          nbt-revise  (adress_issues)
...
cp -r nbt-review ~/.codex/skills/
cp -r nbt-revise  ~/.codex/skills/

# 2. (Optional) point nbt-review / nbt-revise at your Zotero skill.
...
#    <install>/nbt-review/SKILL.md and <install>/nbt-revise/SKILL.md
...
#    Reference resolution is read-only by default. nbt-revise may edit a
...
```bash
python3 tests/validate_skill.py    --skill-root . --run-dir /tmp/nbt_validate   # 40 checks
python3 tests/probe_regressions.py --skill-root . --run-dir /tmp/nbt_probe      # 14 probes
python3 tests/probe_hardcases.py   --skill-root . --run-dir /tmp/nbt_hard       # 4 hard cases
```
...
| `findings.json` | machine-readable findings (8 fields each) + artifacts + coverage — the canonical input to nbt-revise |
...
`nbt-review` copy. If that directory is read-only (typical for an installed
...
- **File naming**: `nbt-revise` gives each revision package ONE version token:
  the 7-character content-hash printed by `nbt-revise/scripts/revision_token.py`
