...
#     NBT_TEST_RUNDIR   directory for <suite>.log, status/<suite>.rc,
#                       status/<suite>.time and the per-suite tmp/ sandbox
#     NBT_TEST_WS       the tree under test (defaults to this script's parent)
#     NBT_TEST_PYTHON   interpreter (defaults to python3)
#
...
ws=${NBT_TEST_WS:-$(dirname -- "$here")}
rundir=${NBT_TEST_RUNDIR:-$(dirname -- "$here")}
python=${NBT_TEST_PYTHON:-python3}
...
PYTHONDONTWRITEBYTECODE=1 \
    "$python" "$ws/.nbt_test/$suite" >"$rundir/$suite.log" 2>&1
rc=$?
end=$(date +%s)
