...
Run:  python3 .nbt_test/test_grading_scheme.py
`NBT_WS` retargets the harness at a baseline copy (pre-change -> red).
"""
...
# A. panel integrity
# =====================================================================
tmp = Path(tempfile.mkdtemp(prefix="nbt_grade_a1_"))
ctx = make_ctx(tmp)
...
tmp = Path(tempfile.mkdtemp(prefix="nbt_grade_a2_"))
...
# B. tie-breaks
# =====================================================================
tmp = Path(tempfile.mkdtemp(prefix="nbt_grade_b1_"))
ctx = make_ctx(tmp)
full_panel(ctx, tie_matrix())
agg = np.aggregate_round(ctx, 2, FIELD)
...
# =====================================================================
tmp = Path(tempfile.mkdtemp(prefix="nbt_grade_b2_"))
...
# =====================================================================
tmp = Path(tempfile.mkdtemp(prefix="nbt_grade_d1_"))
ctx = make_ctx(tmp)
full_panel(ctx, tie_matrix())
agg = np.aggregate_round(ctx, 2, FIELD)
...
tmp1b = Path(tempfile.mkdtemp(prefix="nbt_grade_d1b_"))
...
tmp1c = Path(tempfile.mkdtemp(prefix="nbt_grade_d1c_"))
...
tmp2 = Path(tempfile.mkdtemp(prefix="nbt_grade_d2_"))
ctx2 = make_ctx(tmp2)
for vid in FIELD:
    others = [w for w in FIELD if w != vid]
    labels = [f"v{i+1}" for i in range(len(others))]
...
for name in ("rewrite", "revise", "integrate"):
    ...
    check(f"D5 the {name} prompt carries the post-edit validation rule",
          ...
          and "nbt_docx_format.py validate" in flat
