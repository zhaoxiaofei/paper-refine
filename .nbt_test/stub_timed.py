...
named by NBT_TIMING_LOG (start/end of the session) and sleeps for a
...
The sleep is chosen by the run-id suffix:
    ..._w<k>        NBT_TIMING_SLEEP_W   (rewrite)
    ..._review      NBT_TIMING_SLEEP_R   (review)
    ..._a<k>_revise NBT_TIMING_SLEEP_V   (revise)
    ..._i<k>        NBT_TIMING_SLEEP_I   (integration)
    ..._judge_...   NBT_TIMING_SLEEP_J   (judge)
falling back to NBT_TIMING_SLEEP (default 0, i.e. no sleep).
"""
...
def append_log(event: str, name: str) -> None:
    path = os.environ.get("NBT_TIMING_LOG")
    if not path:
        return
...
def intentionally_fail(name: str) -> bool:
    """Fail this attempt once (NBT_TIMING_FAIL_ONCE) or always (…_ALWAYS_FAIL).
    ...
    """
    always = os.environ.get("NBT_TIMING_ALWAYS_FAIL") or ""
    if always and always in name:
        append_log("fail", name)
        return True
    once = os.environ.get("NBT_TIMING_FAIL_ONCE") or ""
    if once and once in name:
        log_path = os.environ.get("NBT_TIMING_LOG") or "/tmp/nbt_timing_fail"
...
def write_bad_marker(name: str) -> bool:
    """Write a marker whose stage/payload is wrong (NBT_TIMING_BAD_MARKER).
    ...
    """
    target = os.environ.get("NBT_TIMING_BAD_MARKER") or ""
...
def main() -> int:
    ...
    name = sb.name
    kind = kind_of(name)
    sleep_s = float(os.environ.get(f"NBT_TIMING_SLEEP_{kind}",
                                   os.environ.get("NBT_TIMING_SLEEP", "0")) or 0)
