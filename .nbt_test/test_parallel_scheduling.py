...
Run:  python3 .nbt_test/test_parallel_scheduling.py
...
def run_round(tmp: Path, *, rewrites: str, revises: str, jobs: int, sleeps: dict,
    ...
    root = tmp / "root"
    log = tmp / "timing.log"
    subprocess.run([sys.executable, str(WS / "nbt_pipeline.py"), "setup",
                    "--source", str(source), "--root", str(root), "--rounds", "1",
                    "--judges", "2", "--rewrites", rewrites, "--revises", revises],
                   capture_output=True, text=True, check=True)
    env = dict(os.environ)
    env["NBT_TIMING_LOG"] = str(log)
    for key, value in sleeps.items():
        env[key] = str(value)
    env.update(extra_env or {})
    proc = subprocess.run([sys.executable, str(WS / "nbt_pipeline.py"), "run",
...
def test_dag_scheduling():
    ...
    check("the review finishes BEFORE the slow rewrites (so a revise can start early)",
          rev["end"] < w1["start"] + sleeps["NBT_TIMING_SLEEP_W"] - 0.3,
...
def test_jobs_cap_is_respected():
    ...
    tmp = scratch("nbt_sched1_")
    sleeps = {"NBT_TIMING_SLEEP": 0.25}
...
def test_dependency_order_is_never_violated():
    ...
    tmp = scratch("nbt_sched2_")
    sleeps = {"NBT_TIMING_SLEEP_W": 0.4, "NBT_TIMING_SLEEP_R": 0.2,
              "NBT_TIMING_SLEEP_V": 0.2, "NBT_TIMING_SLEEP_I": 0.2,
              "NBT_TIMING_SLEEP_J": 0.1}
...
def test_retry_policy_survives_the_scheduler():
    ...
    tmp = scratch("nbt_sched3_")
    sleeps = {"NBT_TIMING_SLEEP": 0.2}
    root, proc, spans = run_round(tmp, rewrites="1", revises="1", jobs=4, sleeps=sleeps,
                                  retries="1",
                                  extra_env={"NBT_TIMING_FAIL_ONCE": "r1_a2_revise"})
    out = proc.stdout + proc.stderr
...
def test_permanent_failure_keeps_the_round_undecided():
    ...
    tmp = scratch("nbt_sched4_")
    sleeps = {"NBT_TIMING_SLEEP": 0.1}
    root, proc, spans = run_round(tmp, rewrites="1", revises="1", jobs=4, sleeps=sleeps,
                                  retries="1",
                                  extra_env={"NBT_TIMING_ALWAYS_FAIL": "r1_a2_revise"})
    out = proc.stdout + proc.stderr
...
def test_broken_marker_is_a_failed_attempt():
    ...
    tmp = scratch("nbt_sched5_")
    sleeps = {"NBT_TIMING_SLEEP": 0.1}
    start = time.time()
    root, proc, spans = run_round(tmp, rewrites="1", revises="1", jobs=4, sleeps=sleeps,
                                  retries="1",
                                  extra_env={"NBT_TIMING_BAD_MARKER": "r1_w1"})
