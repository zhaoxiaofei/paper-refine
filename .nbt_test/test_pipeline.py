#!/usr/bin/env python3
"""Validation harness for the nbt_pipeline design fixes.

Run:  python3 .nbt_test/test_pipeline.py
...
WORKSPACE = Path(os.environ.get("NBT_WS") or Path(__file__).resolve().parent.parent)
spec = importlib.util.spec_from_file_location("nbtp", WORKSPACE / "nbt_pipeline.py")
...
# ---------------------------------------------------------------- prompts
def test_prompts():
    sb = Path("/tmp/nbt_validate/sandbox")
    builders = {
    ...
    split_ctx = make_ctx(Path("/tmp/nbt_validate_split"))
    ...
    skills = WORKSPACE / "nbt-skills"
    sweeps = (skills / "nbt-review" / "references" / "sweeps.md").read_text(encoding="utf-8")
    ledger = (skills / "nbt-revise" / "references" / "ledger.md").read_text(encoding="utf-8")
...
def test_selection():
    ...
    tmp = Path(tempfile.mkdtemp(prefix="nbt_sel_base_"))
    pairs = {
    ...
    tmp = Path(tempfile.mkdtemp(prefix="nbt_sel_mean_"))
    pairs = {
    ...
    tmp = Path(tempfile.mkdtemp(prefix="nbt_sel_cap_"))
    ...
    tmp = Path(tempfile.mkdtemp(prefix="nbt_sel_ph_"))
    pairs = symmetric_matrix({}, 0)
    ctx, agg = build_round2_ctx(tmp, pairs)
...
def test_structured_output_exemption():
    tmp = Path(tempfile.mkdtemp(prefix="nbt_so_"))
    ctx = make_ctx(tmp)
...
def test_corpus_rules():
    tmp = Path(tempfile.mkdtemp(prefix="nbt_corpus_"))
    ctx = make_ctx(tmp)
    sb = ctx.root / "runs/r1_a2_revise"
    rev = sb / "revised"
...
def test_panel_gaps_cleared():
    tmp = Path(tempfile.mkdtemp(prefix="nbt_fin_"))
    ctx = make_ctx(tmp)
