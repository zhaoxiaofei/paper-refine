...
are per-suite), so they can run at once:
    ...
    python3 .nbt_test/run_all.py                 # GNU parallel, jobs = #cores
    python3 .nbt_test/run_all.py -j 4            # cap the parallelism
    python3 .nbt_test/run_all.py -j 1            # exactly the old sequential loop
    python3 .nbt_test/run_all.py --only test_pipeline.py test_docx_format.py
...
def run_dir_under(path: str = None) -> Path:
    ...
    if path:
        ...
        return p
    return Path(tempfile.mkdtemp(prefix="nbt-tests-"))


def run_one(suite: str, rundir: Path, python: str) -> None:
    env = dict(os.environ,
               NBT_TEST_RUNDIR=str(rundir), NBT_TEST_WS=str(WS), NBT_TEST_PYTHON=python)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run the .nbt_test suites in parallel.")
    ...
    python = os.environ.get("NBT_TEST_PYTHON") or sys.executable
