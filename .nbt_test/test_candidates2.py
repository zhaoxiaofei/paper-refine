...
Run:  python3 .nbt_test/test_candidates2.py
...
round-2 patches. `NBT_WS` / `NBT_SKILLS` retarget it at a baseline copy:

    NBT_WS=/tmp/nbt_baseline2 NBT_SKILLS=/tmp/nbt_baseline2 \
        python3 .nbt_test/test_candidates2.py     # 11 checks fail there
"""
...
WS = Path(os.environ.get("NBT_WS") or DEFAULT_WS)
...
# pack; NBT_SKILLS still wins when a caller wants to point at one explicitly.
