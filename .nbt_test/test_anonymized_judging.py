...
Run:  python3 .nbt_test/test_anonymized_judging.py
...
`NBT_WS` retargets the suite at a baseline copy of the tree (red there).
"""
...
spec = importlib.util.spec_from_file_location("nbt_anon", str(WS / "nbt_pipeline.py"))
nb = importlib.util.module_from_spec(spec)
sys.modules["nbt_anon"] = nb
...
def test_console_timestamps():
    ...
    (bad / "root" / "state.json").write_text(json.dumps(
        {"version": nb.STATE_VERSION, "runs": {"r1_x": {"kind": "judge", "round": 1}},
         "rounds": {}, "pinned": [], "log": []}))
    err = subprocess.run([sys.executable, str(WS / "nbt_pipeline.py"), "status",
    ...
    helptext = subprocess.run([sys.executable, str(WS / "nbt_pipeline.py"), "--help"],
