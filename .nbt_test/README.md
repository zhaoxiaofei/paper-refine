...
| `test_artifact_repair.py` | `--strict-artifacts fix` (the scoped ARTIFACT-REPAIR session), per stage: the three-valued policy (`on`/`fix`/`off` and the aliases), a repairable bookkeeping problem for EVERY stage against a content/contract failure that must stay a plain failure, each profile's byte-level scope and pinned evidence (decision-table rows, headers and prose; an auditor's dispositions/`adds`; a judge's ledger, comparison set and coverage entries; a package's manuscript files), each stage's prompt, and end-to-end runs with `stub_repair_agent.py` — a broken audit sheet, deleted rewrite report, dropped revision-ledger row, deleted integration ledger, stripped judge `checks` map and the M1b long-form gate (undisposed residue rows, the class that failed the 2026-09-22 roots) are each repaired and the run completes; a repair that edits the package's manuscript is rejected; an ADOPTED run (a resume over a finished sandbox) is adopted and repaired without re-running the stage; a repair that clears nothing fails like any other attempt; and `on`/`off` never and always repair, respectively. |
...
| `test_integrate_diagnostics.py` | The three defects of the 2026-09-22 `r1_i3` integration failure: (1) the integration prompt demanded reading four complete corpora in full and forbade diff tools, which ended two attempts at 366k/204k tokens without one bookkeeping file -- the prompt now starts from the orchestrator's mechanical difference index and still forbids deciding a row from diff text alone, while `donor_diff_pack()` seeds that index (`integrated/work/DIFF_MATRIX.md` per-donor file matrix with sizes + `integrated/work/diffs/<donor>_vs_self.md` with unified diffs of the text sources and aligned paragraph diffs of every Word document, plus `work/DONOR_DIFFS.md`; payload only, `work/` scratch excluded, artifact-citable paths) and the ledger rule asks for bookkeeping as the session goes; (2) the ledger READER took the file's FIRST table, so a session whose DIFF_LEDGER.md opened with an orientation table had those rows read as the ledger and failed with "4 ledger row(s) carry no `artifact`" -- `integration_ledger_rows()` now selects the table that carries the ledger's own columns (donor/artifact/size/finding effect) and falls back to the first table only when the file has none; (3) the repair-scope guard counted `__pycache__/*.pyc` (written by RUNNING the seeded helper scripts) as an out-of-scope edit and failed a repair that had cleared every problem it was given -- bytecode caches are now ignored while a real out-of-scope create is still caught. |
