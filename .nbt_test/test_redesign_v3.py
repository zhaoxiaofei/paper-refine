...
Run:  python3 .nbt_test/test_redesign_v3.py
`NBT_WS` retargets the suite at a baseline copy (red before the redesign).
"""
...
def test_prompts_and_policy():
    ...
    tmp = Path(tempfile.mkdtemp(prefix="nbt_v3_pol_"))
    TMPDIRS.append(tmp)
    ctx = np.Ctx(tmp)
...
def test_field_wide_ranking_and_reported_margin():
    ...
    tmp = Path(tempfile.mkdtemp(prefix="nbt_v3_rank_"))
    TMPDIRS.append(tmp)
    ctx = np.Ctx(tmp)
...
def test_second_pass():
    ...
    check("the inserted reading-order block adds no provenance vocabulary",
          not _hits, str(_hits))
    tmp = Path(tempfile.mkdtemp(prefix="nbt_v3_stop_"))
    TMPDIRS.append(tmp)
    source = tmp / "source"
    ...
    run = subprocess.run([sys.executable, str(WS / "nbt_pipeline.py"), "run",
